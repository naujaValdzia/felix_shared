...
## Resolve the JAVA_HOME
#
#  JAVA_HOME points to where the Java installation is located
if [ "${JAVA_HOME}" = "" ]
then
    JAVA_HOME="$DLC"/jdk
    export JAVA_HOME
    if [ "${JAVA_HOME}" = "" ]
    then
        echo "Cannot resolve JAVA_HOME environment"
        exit 1
    fi
fi

## Resolve the JRE_HOME
#
#  JRE_HOME points to where the JRE installation is located
if [ "${JRE_HOME}" = "" ]
then
    JRE_HOME="$DLC"/jre
    export JRE_HOME
...
DEPLOYMGRCLASS="com.progress.appserv.manager.util.deploySOAP"
...
DEPLOY_SERVICE()
{
  ...
  then
    ...
    echo "Updating."
    $JAVA_HOME/bin/java -Dorg.apache.cxf.Logger=org.apache.commons.logging.impl.NoOpLog -DInstall.Dir=$SCHEMADIR -cp $CLASSPATH -Dcatalina.home=$CATALINA_HOME -Dcatalina.base=$CATALINA_BASE -Dpsc.as.host.name=$PSC_DNS_NAME -Dpsc.as.http.port=$PSC_HTTP_PORT  -Dpsc.as.https.port=$PSC_HTTPS_PORT $DEPLOYMGRCLASS -artifact $SRCDESCLOC -service $APPNAME -deploy
    EXIT_CODE=$?
  fi
}
...
UNDEPLOY_SERVICE()
{
    $JAVA_HOME/bin/java -Dorg.apache.cxf.Logger=org.apache.commons.logging.impl.NoOpLog -DInstall.Dir=$SCHEMADIR -cp $CLASSPATH -Dcatalina.home=$CATALINA_HOME -Dcatalina.base=$CATALINA_BASE -Dpsc.as.host.name=$PSC_DNS_NAME -Dpsc.as.http.port=$PSC_HTTP_PORT  -Dpsc.as.https.port=$PSC_HTTPS_PORT $DEPLOYMGRCLASS -artifact $SRCDESCLOC -service $APPNAME -undeploy
    EXIT_CODE=$?
