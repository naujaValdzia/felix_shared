...
if [ "${JAVA_HOME}" = "" -o  ! -d "${JAVA_HOME}" ]
then
	echo "Cannot resolve JAVA_HOME environment. JAVA_HOME=${JAVA_HOME}"
	exit 1
fi
...
if [ "${JRE_HOME}" = "" -o ! -d "${JRE_HOME}" ]
then
	JRE_WARNING=" !!!WARNING. Could not find this directory"
fi
